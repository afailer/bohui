<template>
    <div class="mytable commondiv spritesbox bohui-tongji" ref="container" style="overflow: auto">
        <span class="sprites-lt"></span>
        <span class="sprites-rt"></span>
        <span class="sprites-lb"></span>
        <span class="sprites-rb"></span>
        <div class="title">{{$store.state.areaName}}人类活动</div>
        <div :id="this.direction+'humanPieChart'" style="width: 100%;height: 25%;min-height: 260px;z-index: 9999999;margin-top: 10px"></div>
...
export default {
    ...
    mounted() {
        if(this.direction === 'left'){
            this.$refs.container.style.left = '10px'
        }else{
            this.$refs.container.style.right = '10px'
        }
    },
    computed: {
        leftTime() {
    ...
    methods: {
        loadData(dateDirection){
            let url = '';
            if(this.direction === 'left' && dateDirection === 'left' && this.$store.state.leftDataTime !== ''){
                url = this.$store.state.bohuiAddress + `/humanAction/speckle?areaCode=${this.$store.state.areaCode}&dataTime=${this.$store.state.leftDataTime}&type=1`;
            }else if(this.direction === 'right' && dateDirection === 'right' && this.$store.state.rightDataTime){
                url = this.$store.state.bohuiAddress + `/humanAction/speckle?areaCode=${this.$store.state.areaCode}&dataTime=${this.$store.state.leftDataTime}&type=1`;
            }
            if(url !== ''){
        ...
        resetChartData(id){
            const myChart = this.$echarts.init(document.getElementById(this.direction+'humanPieChart'));
        ...
        initBarChart(){
            this.$nextTick(() => {
                this.chartIds.forEach(id => {
                    const myChart = this.$echarts.init(document.getElementById(id+this.direction));
